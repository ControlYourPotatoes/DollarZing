...
const useSimulationStore = create<SimulationStore>()((set, get) => ({
  ...
  setCashOutStrategy1: (strategy: CashOutStrategy) =>
    set({ cashOutStrategy1: strategy }),
  setCashOutStrategy2: (strategy: CashOutStrategy) =>
    set({ cashOutStrategy2: strategy }),
  ...
  calculateWeeklyStats: (chartData: ChartDataPoint[]): WeeklyStats[] => {
    ...
    for (let i = 0; i < chartData.length; i += 7) {
      ...
      const weekStat: WeeklyStats = {
        ...
        totalCharity: weekData.reduce(
          (sum, day) => sum + day.charityContributions,
          0
        ),
        totalPlatformEarnings: weekData.reduce(
          (sum, day) => sum + day.platformEarnings,
          0
        ),
        totalJackpotWinners: weekData.reduce(
          (sum, day) => sum + day.jackpotWinners,
          0
        ),
        totalGamesPlayed: weekData.reduce(
          (sum, day) => sum + day.gamesPlayed,
          0
        ),
        activePlayersGrowth:
          ((lastDayOfWeek.activePlayers - firstDayOfWeek.activePlayers) /
            firstDayOfWeek.activePlayers) *
          100,
        averageDailyGames:
          weekData.reduce((sum, day) => sum + day.gamesPlayed, 0) /
          weekData.length,
      };
  ...
  simulateDay: (
    prevState: GameState,
    { cashOutStrategy, adoptionRate }: SimulationProps
  ): GameState => {
    ...
    const getBillsPerDay = (
      strategy: CashOutStrategy,
      rng: SeededRandom
    ): number => {
      const range =
        BILLS_PER_DAY_RANGE[strategy] || BILLS_PER_DAY_RANGE.average;
    ...
    if (adoptionRate === 0) {
      ...
      const effectiveGrowthRate =
        (baseGrowthRate + randomFactor) * (1 + timeFactor);
      ...
      const randomFactor =
        Math.floor(rng.next() * baseAdoptedUsers * 0.2) -
    ...
    LEVELS.forEach((level) => {
      ...
      if (level === 512) {
        ...
        winningsToDistribute = cashOutPlayers * level * 1.8; // Match CryptoZing's winnings structure
      }
      ...
      currentDayLevelData.push({
        ...
        cashOutProbability:
          level === 512 ? 1 : getCashOutProbability(level, cashOutStrategy),
        cashOutPlayers,
        losingPlayers,
      ...
      gamesAtCurrentLevel =
        level === 512
          ? 0
          : Math.floor(gamesAtCurrentLevel / 2) - cashOutPlayers;
    });

    const totalMoneyInSystem = initialGames + dailyPlatformEarnings; // Each initial game is $1
    const totalDistributed =
      dailyCharity + dailyGovernmentEarnings + dailyPlayerWinnings;
