...
import AdjustableChartComponent from "./OldAdjustableChart";
import { GameState, ChartDataPoint, CashOutStrategy } from "@/types/types";

const LEVELS = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512];
const PLATFORM_FEE = 0.2;
const GOVERNMENT_SHARE = 0.4;
const CHARITY_SHARE = 0.2;
const PLAYER_SHARE = 0.4;
...
const logDetailedSimulation = (
  ...
  LEVELS.forEach((level, _) => {
    ...
    const winningsAtLevel = cashOutWinners * level * 1.8; // Match CryptoZing's winnings structure
  ...
  console.log(
    `Total Calculated Winnings: $${totalCalculatedWinnings.toFixed(2)}`
  );
  console.log(
    `Actual Total Winnings: $${(
      dailyPlayerWinnings +
  ...
  console.log(
    `Discrepancy: $${(
      totalCalculatedWinnings -
      (dailyPlayerWinnings + dailyGovernmentEarnings + dailyCharity)
    ).toFixed(2)}`
  );
...
const GameSimulation = () => {
  ...
  const [cashOutStrategy1, setCashOutStrategy1] =
    useState<CashOutStrategy>("average");
  const [cashOutStrategy2, setCashOutStrategy2] =
    useState<CashOutStrategy>("average");
    ...
    (
      ...
      if (adoptionRate === 0 || adoptionRate === undefined) {
        ...
        const effectiveGrowthRate =
          (baseGrowthRate + randomFactor) * (1 + timeFactorb);
        newActivePlayers = Math.floor(
          prevState.activePlayers * (1 + effectiveGrowthRate)
        );
      } else {
        // Adoption rate growth with randomness
        const baseAdoptedUsers = Math.floor(
          PANAMA_LOTTERY_USERS * adoptionRate
        );
        const randomFactor =
          Math.floor(Math.random() * baseAdoptedUsers * 0.2) -
          Math.floor(baseAdoptedUsers * 0.1);
      ...
      console.log(
        `Day ${newDay}: ${
          adoptionRate === 0 ? "Natural" : "Adoption"
        } growth - Total players: ${
          prevState.totalPlayers
        }, Active players: ${newActivePlayers}, Cashout strategy: ${cashOutStrategy}`
      );
      ...
      LEVELS.forEach((level) => {
        ...
        const cashOutProbability = getCashOutProbability(
          level,
          cashOutStrategy
        );
        const cashOutWinners = Math.floor(
          potentialWinnersAtLevel * cashOutProbability
        );
        const payoutAtLevel = Math.min(
          cashOutWinners * level * 2,
          remainingPot
        );
      ...
      const totalDailyGames = Object.values(levelData).reduce(
        (sum, games) => sum + games,
        0
      );
  ...
  useEffect(() => {
    ...
    if (isRunning) {
      timer = setInterval(() => {
        setGameState1((prevState) =>
          simulateDay(prevState, cashOutStrategy1, 0)
        ); // Always 0 for natural growth
        setGameState2((prevState) =>
          simulateDay(prevState, cashOutStrategy2, adoptionRate2)
        );
      }, 1000);
    }
