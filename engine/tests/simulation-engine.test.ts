...
import { SimulationParameters } from "../src/types/simulation-types";
...
describe("Core Simulation Engine", () => {
  describe("SimulationEngine", () => {
    ...
    it("should generate yearly dataset within performance requirements", async () => {
      ...
      expect(dataset.metadata.generationDurationMs).toBeGreaterThan(0); // Should have some duration
    });
    ...
    it("should maintain financial consistency across all days", async () => {
      ...
      dataset.dailySnapshots.forEach((snapshot, index) => {
        ...
        const totalRevenue =
          financialMetrics.platformEarnings +
    ...
    it("should generate consistent results with same seed", async () => {
      ...
      expect(dataset1.dailySnapshots[0].financialMetrics.gamesPlayed).toBe(
        dataset2.dailySnapshots[0].financialMetrics.gamesPlayed
      );
      expect(dataset1.dailySnapshots[0].playerMetrics.totalPlayers).toBe(
        dataset2.dailySnapshots[0].playerMetrics.totalPlayers
      );
    });
  ...
  describe("PlayerBehaviorModel", () => {
    ...
    it("should calculate cash-out probabilities correctly", () => {
      ...
      expect(behaviorModel.getCashOutProbability(1, "low")).toBeGreaterThan(
        0.8
      );
      ...
      expect(behaviorModel.getCashOutProbability(10, "high")).toBeGreaterThan(
        0.5
      );

      // Average strategy: consistent probabilities
      expect(behaviorModel.getCashOutProbability(1, "average")).toBeCloseTo(
        0.3,
        1
      );
      expect(behaviorModel.getCashOutProbability(10, "average")).toBeCloseTo(
        0.3,
        1
      );
    });
    ...
    it("should model player journeys with progression tracking", () => {
      const journey = behaviorModel.simulatePlayerJourney(
        "player-001",
        "high",
        30
      );
  ...
  describe("GrowthCalculator", () => {
    ...
    it("should calculate organic growth rates", () => {
      const baseGrowth = growthCalculator.calculateOrganicGrowth(
        1,
        1000,
        1.2,
        0.12345
      ...
      const dayTenGrowth = growthCalculator.calculateOrganicGrowth(
        10,
        1000,
        1.2,
        0.12345
    ...
    it("should calculate adoption growth correctly", () => {
      const adoptionRate = 0.1; // 10% adoption

      const adoptedUsers = growthCalculator.calculateAdoptionGrowth(
        5,
        1000,
        adoptionRate,
        0.12345
      ...
      expect(adoptedUsers).toBeGreaterThanOrEqual(1000); // Should be monotonic
    });
    ...
    it("should apply growth multipliers correctly", () => {
      ...
      const multipliedGrowth = growthCalculator.calculateOrganicGrowth(
        5,
        1000,
        2.0,
        0.12345
    ...
    it("should maintain player count constraints", () => {
      const currentUsers = 950000;
      const nearMaxGrowth = growthCalculator.calculateAdoptionGrowth(
        50,
        currentUsers,
        0.1,
        0.12345
      ...
      expect(nearMaxGrowth).toBeGreaterThanOrEqual(currentUsers); // Should be monotonic
    });
  });
  ...
  describe("LevelProgressionCalculator", () => {
    ...
    it("should calculate level statistics correctly", () => {
      const stats = progressionCalc.calculateLevelStatistics(
        5,
        1000,
        "average",
        0.12345
    ...
    it("should identify bottlenecks at higher levels", () => {
      const level3Stats = progressionCalc.calculateLevelStatistics(
        3,
        1000,
        "low",
        0.12345
      );
      const level8Stats = progressionCalc.calculateLevelStatistics(
        8,
        100,
        "low",
        0.12345
      ...
      expect(level8Stats.bottleneckScore).toBeGreaterThanOrEqual(
        level3Stats.bottleneckScore
      );
    });

    it("should handle edge cases at level 10 (jackpot level)", () => {
      const jackpotStats = progressionCalc.calculateLevelStatistics(
        10,
        50,
        "high",
        0.12345
  ...
  describe("RevenueDistributionCalculator", () => {
    ...
    it("should calculate revenue breakdown correctly", () => {
      ...
      expect(breakdown.players.percentage).toBeCloseTo(0.6, 2);

      const totalAmount =
        breakdown.platform.amount +
        breakdown.charity.amount +
        breakdown.players.amount;
      expect(totalAmount).toBeCloseTo(1000, 2);
    });

    it("should handle different transaction types", () => {
      const cashoutBreakdown = revenueCalc.calculateRevenueBreakdown(
        1000,
        "cashout"
      );
      ...
      expect(lossBreakdown.players.percentage).toBeLessThan(
        cashoutBreakdown.players.percentage
      );
    });

    it("should maintain percentage consistency", () => {
      for (const amount of [100, 500, 1000, 5000]) {
        const breakdown = revenueCalc.calculateRevenueBreakdown(
          amount,
          "cashout"
        );
        const totalPercentage =
          breakdown.platform.percentage +
          breakdown.charity.percentage +
          breakdown.players.percentage;
  ...
  describe("Parameter Validation", () => {
    ...
    it("should reject invalid adoption rates", () => {
      ...
      expect(() => new SimulationEngine(invalidParams)).toThrow(
