...
import { GameResult, CashOutStrategy } from '../src/types/virtual-dollar-engine';
describe('RunOrchestrator - Run Completion and New Run Logic Tests', () => {
    let orchestrator;
    beforeEach(() => {
        orchestrator = new RunOrchestrator(undefined, undefined, 0.15); // 15% charity
    });
    describe('Player Initialization and Fund Management', () => {
        test('should initialize player with funds and strategy', () => {
            orchestrator.initializePlayer('player-1', 20.00, CashOutStrategy.BALANCED);
            expect(orchestrator.getPlayerFunds('player-1')).toBe(20.00);
            expect(orchestrator.getPlayerStrategy('player-1')).toBe(CashOutStrategy.BALANCED);
        });
        test('should track multiple players independently', () => {
            orchestrator.initializePlayer('player-1', 20.00, CashOutStrategy.CONSERVATIVE);
            orchestrator.initializePlayer('player-2', 50.00, CashOutStrategy.AGGRESSIVE);
            orchestrator.initializePlayer('player-3', 5.00, CashOutStrategy.BALANCED);
            expect(orchestrator.getPlayerFunds('player-1')).toBe(20.00);
            expect(orchestrator.getPlayerFunds('player-2')).toBe(50.00);
            expect(orchestrator.getPlayerFunds('player-3')).toBe(5.00);
            expect(orchestrator.getPlayerStrategy('player-1')).toBe(CashOutStrategy.CONSERVATIVE);
            expect(orchestrator.getPlayerStrategy('player-2')).toBe(CashOutStrategy.AGGRESSIVE);
            expect(orchestrator.getPlayerStrategy('player-3')).toBe(CashOutStrategy.BALANCED);
        });
        test('should allow fund additions', () => {
            orchestrator.initializePlayer('player-1', 10.00, CashOutStrategy.BALANCED);
            orchestrator.addPlayerFunds('player-1', 15.00);
            expect(orchestrator.getPlayerFunds('player-1')).toBe(25.00);
        });
    });
    describe('New Run Creation', () => {
        ...
        test('should create new run when player has sufficient funds', () => {
            ...
            expect(orchestrator.getPlayerFunds('player-1')).toBe(18.90);
        });
        test('should reject new run when player has insufficient funds', () => {
            orchestrator.initializePlayer('broke-player', 0.50, CashOutStrategy.BALANCED);
            const request = {
            ...
            expect(orchestrator.getPlayerFunds('broke-player')).toBe(0.50); // No funds deducted
        });
        test('should update player strategy when creating new run', () => {
            ...
            expect(orchestrator.getPlayerStrategy('player-1')).toBe(CashOutStrategy.AGGRESSIVE);
        });
    });
    describe('Game Result Processing and Run Completion', () => {
        let testRun;
        beforeEach(() => {
            orchestrator.initializePlayer('player-1', 20.00, CashOutStrategy.AGGRESSIVE); // Use aggressive for jackpot test
        ...
        test('should handle run completion on loss', () => {
            const completionEvent = orchestrator.processGameResult(testRun, GameResult.LOSS);
        ...
        test('should handle run completion on jackpot', () => {
            ...
            for (let level = 1; level <= 10; level++) {
                completionEvent = orchestrator.processGameResult(testRun, GameResult.WIN);
                if (level < 10) {
            ...
            const expectedFunds = 18.90 + (1024 * 0.85);
        ...
        test('should handle cash-out completion', () => {
            ...
            orchestrator.initializePlayer('cash-out-player', 20.00, CashOutStrategy.CONSERVATIVE);
            ...
            const completionEvent = orchestrator.processGameResult(cashOutRun, GameResult.WIN); // Level 3 → 4, $8
            expect(completionEvent).not.toBeNull();
            ...
            const expectedFunds = 18.90 + (8 * 0.85);
            expect(orchestrator.getPlayerFunds('cash-out-player')).toBe(expectedFunds);
        });
    ...
    describe('Player Run Context', () => {
        ...
        test('should provide accurate player run context with no runs', () => {
            ...
            expect(context.availableFunds).toBe(20.00);
        ...
        test('should track active runs in context', () => {
            ...
            expect(context.availableFunds).toBe(18.90);
            expect(context.canCreateNewRun).toBe(true);
        });
        test('should track completed runs and earnings', () => {
            // Initialize a new player with conservative strategy for this test
            orchestrator.initializePlayer('conservative-player', 20.00, CashOutStrategy.CONSERVATIVE);
            const run1 = orchestrator.createNewRun({
        ...
        test('should correctly determine when player cannot create new run', () => {
            orchestrator.initializePlayer('poor-player', 1.00, CashOutStrategy.BALANCED);
    ...
    describe('Multiple Player Management', () => {
        ...
        test('should track total funds across all players', () => {
            expect(orchestrator.getTotalPlayerFunds()).toBe(75.00);
            // Create some runs
            ...
            expect(orchestrator.getTotalPlayerFunds()).toBe(72.80);
        });
    ...
    describe('Run Completion and Fund Validation for New Runs', () => {
        test('should handle player becoming unable to create new runs after losses', () => {
            orchestrator.initializePlayer('player-1', 2.20, CashOutStrategy.BALANCED); // Exactly 2 games worth
            // Create first run
            const run1 = orchestrator.createNewRun({
                playerId: 'player-1',
                cashOutStrategy: CashOutStrategy.BALANCED,
            ...
            expect(orchestrator.getPlayerFunds('player-1')).toBe(1.10); // One game left
            // Lose first run
            ...
            expect(orchestrator.getPlayerFunds('player-1')).toBe(0.00); // No funds left
            // Lose second run
        ...
        test('should enable new runs after successful cash-out provides funds', () => {
            orchestrator.initializePlayer('limited-player', 1.10, CashOutStrategy.CONSERVATIVE); // Just enough for 1 game
            const run = orchestrator.createNewRun({
                playerId: 'limited-player',
                cashOutStrategy: CashOutStrategy.CONSERVATIVE,
                fundingSource: 'DONATION'
            });
            expect(orchestrator.getPlayerFunds('limited-player')).toBe(0.00);
...
//# sourceMappingURL=run-orchestrator.test.js.map