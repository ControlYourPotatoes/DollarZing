...
import { VirtualDollarManager, DollarState } from '../src/types/virtual-dollar-types';
...
describe('GameMatchingEngine', () => {
    ...
    describe('Game Resolution using Algorithmic Scores', () => {
        ...
        it('should handle score ties deterministically', () => {
            ...
            const results = Array.from({ length: 5 }, () => gameMatchingEngine.resolveGame(game.id, '2025-08-21'));
    ...
    describe('Concurrent Game Handling', () => {
        it('should handle multiple concurrent games without conflicts', async () => {
            ...
            const resolutionPromises = matchResult.gamesCreated.map(game => Promise.resolve(gameMatchingEngine.resolveGame(game.id, '2025-08-21')));
    ...
    describe('Game Event System', () => {
        it('should emit events during game lifecycle', () => {
            const events = [];
            gameMatchingEngine.on('gameCreated', (event) => events.push(event));
            gameMatchingEngine.on('gameResolved', (event) => events.push(event));
            const dollar1 = virtualDollarManager.createVirtualDollar('player1');
            const dollar2 = virtualDollarManager.createVirtualDollar('player2');
            [dollar1, dollar2].forEach(dollar => {
                dollar.currentLevel = 1;
        ...
        it('should provide detailed event data for analytics', () => {
            let gameCreatedEvent = null;
            let gameResolvedEvent = null;
            gameMatchingEngine.on('gameCreated', (event) => {
                gameCreatedEvent = event;
            });
            gameMatchingEngine.on('gameResolved', (event) => {
...
//# sourceMappingURL=game-matching-engine.test.js.map