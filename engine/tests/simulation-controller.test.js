// SimulationController Tests - Task 7.1
// Tests for simulation orchestration with all components including PlayerBalanceManager
import { SimulationController } from '../src/types/simulation-controller';
...
describe('SimulationController', () => {
    ...
    beforeEach(() => {
        ...
        config = {
            ...
            initialDonationAmount: 20.00, // $20 starting donation
    ...
    describe('Constructor and Initialization', () => {
        it('should create controller with valid configuration', () => {
            expect(controller).toBeInstanceOf(SimulationController);
        ...
        it('should initialize all required components', () => {
            ...
            expect(components.playerBalanceManager).toBeInstanceOf(PlayerBalanceManager);
        ...
        it('should validate configuration parameters', () => {
            ...
            expect(() => new SimulationController(invalidConfig)).toThrow('Duration must be at least 1 day');
            const invalidPlayerCount = { ...config, initialPlayerCount: -1 };
            expect(() => new SimulationController(invalidPlayerCount)).toThrow('Initial player count must be positive');
            const invalidCharity = { ...config, charityPercentage: 1.5 };
            expect(() => new SimulationController(invalidCharity)).toThrow('Charity percentage must be between 0 and 1');
        });
        it('should handle player strategy distribution validation', () => {
            ...
            expect(() => new SimulationController(invalidStrategies))
                .toThrow('Player strategy percentages must sum to 1.0');
        });
    });
    describe('Player Initialization - Task 7.3', () => {
        ...
        it('should distribute cash-out strategies according to configuration', () => {
            ...
            const expectedConservative = Math.round(config.playerStrategies[CashOutStrategy.CONSERVATIVE] * config.initialPlayerCount);
            const expectedBalanced = Math.round(config.playerStrategies[CashOutStrategy.BALANCED] * config.initialPlayerCount);
            const expectedAggressive = Math.round(config.playerStrategies[CashOutStrategy.AGGRESSIVE] * config.initialPlayerCount);
            expect(strategyCounts.get(CashOutStrategy.CONSERVATIVE)).toBeCloseTo(expectedConservative, 5);
            expect(strategyCounts.get(CashOutStrategy.BALANCED)).toBeCloseTo(expectedBalanced, 5);
            expect(strategyCounts.get(CashOutStrategy.AGGRESSIVE)).toBeCloseTo(expectedAggressive, 5);
        });
        it('should handle player activation and retirement logic', () => {
            ...
            balanceManager.processGameFee(testPlayer.id, 21.00); // More than they have
    ...
    describe('Error Handling and Validation', () => {
        it('should handle simulation timeout gracefully', async () => {
            ...
            const timeoutController = new SimulationController(shortTimeoutConfig);
        ...
        it('should validate all simulation parameters', () => {
            ...
            configs.forEach(invalidConfig => {
                expect(() => new SimulationController(invalidConfig)).toThrow();
            });
        });
        it('should provide meaningful error messages', () => {
            try {
                new SimulationController({ ...config, durationDays: 0 });
            }
            catch (error) {
                expect(error instanceof Error).toBe(true);
                expect(error.message).toContain('Duration must be at least 1 day');
            }
        });
    });
...
describe('SimulationProgress and Reporting - Task 7.7', () => {
    ...
    it('should calculate accurate completion estimates', async () => {
        const progressUpdates = [];
        controller.initializePlayers();
        await controller.runSimulation((progress) => progressUpdates.push(progress));
        // Verify progression is monotonic
        for (let i = 1; i < progressUpdates.length; i++) {
            expect(progressUpdates[i].completionPercentage).toBeGreaterThanOrEqual(progressUpdates[i - 1].completionPercentage);
        }
...
describe('Simulation Results and Export - Task 7.11', () => {
    ...
    it('should generate comprehensive simulation results', async () => {
        ...
        expect(results.revenueStats.totalCharityContributions).toBeGreaterThanOrEqual(0);
    ...
    it('should include player balance data in export', async () => {
        ...
        const totalFunds = results.playerStats.totalDonationsFunds +
    ...
    it('should provide detailed revenue breakdown', async () => {
        ...
        expect(revenueStats.revenuePerGame).toBe(0.20); // 20c per game
    ...
    it('should format data for external consumption', async () => {
        ...
        expect(parsed.playerStats.totalPlayers).toBe(results.playerStats.totalPlayers);
    });
});
//# sourceMappingURL=simulation-controller.test.js.map