...
describe('ScoringEngine', () => {
    ...
    describe('Score Consistency Across Multiple Calculations', () => {
        it('should maintain consistency over multiple calculation sessions', () => {
            ...
            const dailySeed = '2025-08-21';
            // First session
            const scores1 = Array.from({ length: 10 }, () => scoringEngine.calculateScore(serialNumber, dailySeed));
            // Create new engine instance (second session)
            const newEngine = new ScoringEngine();
            const scores2 = Array.from({ length: 10 }, () => newEngine.calculateScore(serialNumber, dailySeed));
        ...
        it('should be deterministic across engine restarts', () => {
            ...
            const originalScores = testCases.map(({ serial, seed }) => scoringEngine.calculateScore(serial, seed));
    ...
    describe('Score Caching and Memoization', () => {
        ...
        it('should manage cache memory efficiently', () => {
            ...
            const scores = serials.map(serial => scoringEngine.calculateScore(serial, dailySeed));
            ...
            const cachedScores = serials.map(serial => scoringEngine.calculateScore(serial, dailySeed));
    ...
    describe('Score Comparison Functionality', () => {
        ...
        it('should handle tie-breaking deterministically', () => {
            ...
            const results = Array.from({ length: 10 }, () => scoringEngine.compareScores(serial1, serial2, dailySeed));
    ...
    describe('Edge Cases and Error Handling', () => {
        ...
        it('should handle null and undefined inputs gracefully', () => {
            expect(() => {
                scoringEngine.calculateScore(null, '2025-08-21');
            }).toThrow();
            expect(() => {
                scoringEngine.calculateScore(undefined, '2025-08-21');
            }).toThrow();
            expect(() => {
                scoringEngine.calculateScore('A12345678B', null);
            }).toThrow();
            expect(() => {
                scoringEngine.calculateScore('A12345678B', undefined);
            }).toThrow();
        });
