...
describe("VirtualDollarManager", () => {
  ...
  describe("Serial Number Generation", () => {
    ...
    it("should generate realistic letter distributions", () => {
      const serials = Array.from({ length: 100 }, () =>
        manager.generateSerialNumber()
      );
  ...
  describe("Dollar Creation", () => {
    ...
    it("should validate player IDs during creation", () => {
      expect(() => manager.createVirtualDollar("")).toThrow(
        "Invalid player ID"
      );
      expect(() => manager.createVirtualDollar(null as any)).toThrow(
        "Invalid player ID"
      );
      expect(() => manager.createVirtualDollar(undefined as any)).toThrow(
        "Invalid player ID"
      );
    });
  });
  ...
  describe("Dollar State Management", () => {
    ...
    it("should transition through valid state progression", () => {
      ...
      expect(
        manager.updateDollarState(dollar.id, DollarState.POOLED).isValid
      ).toBe(true);
      expect(
        manager.updateDollarState(dollar.id, DollarState.IN_GAME).isValid
      ).toBe(true);
      expect(
        manager.updateDollarState(dollar.id, DollarState.WON).isValid
      ).toBe(true);
      expect(
        manager.updateDollarState(dollar.id, DollarState.CASHED_OUT).isValid
      ).toBe(true);
    });
    ...
    it("should prevent invalid state transitions", () => {
      ...
      expect(result.errors).toContain(
        "Invalid state transition from CREATED to WON"
      );
    });
    ...
    it("should prevent transitions from final states", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Cannot transition from final state LOST"
      );
    });
  });
  ...
  describe("History and Tracking", () => {
    ...
    it("should include timestamps in state history", () => {
      ...
      const history = manager.getDollarStateHistory(dollar.id);
      const pooledTransition = history.find(
        (h) => h.state === DollarState.POOLED
      );

      expect(pooledTransition).toBeDefined();
      expect(pooledTransition!.timestamp.getTime()).toBeGreaterThanOrEqual(
        beforeUpdate
      );
      expect(pooledTransition!.timestamp.getTime()).toBeLessThanOrEqual(
        afterUpdate
      );
    });
  });
  ...
  describe("Error Handling and Validation", () => {
    it("should validate dollar state updates for non-existent dollars", () => {
      const result = manager.updateDollarState(
        "non-existent",
        DollarState.POOLED
      );
      expect(result.isValid).toBe(false);
    ...
    it("should prevent creation with duplicate serial numbers", () => {
      ...
      vi.spyOn(manager, "generateSerialNumber").mockReturnValue(fixedSerial);

      const dollar1 = manager.createVirtualDollar("player1");
      expect(() => manager.createVirtualDollar("player2")).toThrow(
