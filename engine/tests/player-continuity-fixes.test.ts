...
import { VirtualDollar, GameSession, BettingLevel, DollarState } from '../src/types/virtual-dollar-engine';
...
describe('Player Continuity Fixes', () => {
  ...
  beforeEach(() => {
    gameSessionFactory = new DirectGameSessionFactory(DEFAULT_PERFORMANCE_CONFIG);
  ...
  describe('Winnings Calculation (Level × 1.8)', () => {
    it('should calculate winnings as level × 1.8 for level 1', () => {
      ...
      const session = gameSessionFactory.create(testDollar1, testDollar2, level);
    ...
    it('should calculate correct winnings for all betting levels 1-10', () => {
      ...
      expectedWinnings.forEach(({ level, expected }) => {
        ...
        const session = gameSessionFactory.create(testDollar1, testDollar2, level as BettingLevel);
  ...
  describe('Game Resolution and Fund Flow', () => {
    it('should properly distribute winnings to winner after game resolution', () => {
      ...
      const resolutionResult = gameMatchingEngine.resolveGame(gameSession.id, dailySeed);
    ...
    it('should update winner VirtualDollar with correct fund tracking', () => {
      ...
      const resolutionResult = gameMatchingEngine.resolveGame(gameSession.id, '2025-09-01');
    ...
    it('should correctly track winnings separately from game funding', () => {
      ...
      const gameCostFromDonation = 1.10;
      const startingDonationBalance = 20.00;
      const gamesAffordableFromDonation = Math.floor(startingDonationBalance / gameCostFromDonation);
  ...
  describe('Dollar State Management', () => {
    it('should properly track dollar state transitions during game lifecycle', () => {
      ...
      // Move to pooled
      const updateResult = dollarManager.updateDollarState(testDollar1.id, DollarState.POOLED);
    ...
    it('should prevent invalid state transitions', () => {
      ...
      const invalidResult = dollarManager.updateDollarState(testDollar1.id, DollarState.IN_GAME);
  ...
  describe('Virtual Dollar Pot System (Task 2)', () => {
    ...
    it('should not deduct fee if pot value is already at or below $0.90', () => {
      const testDollar = dollarManager.createVirtualDollar('test-player');
      testDollar.potValue = 0.80; // Set to below fee threshold
      
      // Move to pooled state - should not deduct fee
      dollarManager.updateDollarState(testDollar.id, DollarState.POOLED);
      
      expect(testDollar.potValue).toBe(0.80); // Should remain unchanged
    });
    ...
    it('should validate pot values during virtual dollar validation', async () => {
      ...
      const { validateVirtualDollar } = await import('../src/types/virtual-dollar-engine');
  ...
  describe('Remove Player Balance Game Fee Charging (Task 3)', () => {
    it('should not charge game fees to player balance during game resolution (Task 3.1)', async () => {
      ...
      expect(mockBalanceManager.addWinProgression).toHaveBeenCalledWith('player1', 1.8);
      expect(mockBalanceManager.loseProgression).toHaveBeenCalledWith('player2');
    });
    ...
    it('should only charge $1.00 for virtual dollar creation in RunOrchestrator (Task 3.2)', () => {
      ...
      runOrchestrator.initializePlayer('test-player', 5.00, 'BALANCED');
      ...
      const remainingFunds = (runOrchestrator as any).playerFunds.get('test-player');
      expect(remainingFunds).toBe(4.00); // $5.00 - $1.00 = $4.00
    });
    ...
    it('should verify player balance remains stable during virtual dollar gameplay (Task 3.5)', async () => {
      ...
      const initialFunds = 10.00;
      runOrchestrator.initializePlayer('stable-player', initialFunds, 'BALANCED');
      ...
      let currentFunds = (runOrchestrator as any).playerFunds.get('stable-player');
      expect(currentFunds).toBe(9.00);
      
      // Simulate multiple game sessions with the virtual dollar
      // (This would happen through the pot system, not player balance)
      run1!.potValue = 0.90; // After platform fee
      run1!.potValue = 1.80; // After winning a game
      run1!.potValue = 0.90; // After losing pot value to another dollar
      
      // Player balance should remain unchanged during gameplay
      currentFunds = (runOrchestrator as any).playerFunds.get('stable-player');
      expect(currentFunds).toBe(9.00); // Still $9.00
      ...
      currentFunds = (runOrchestrator as any).playerFunds.get('stable-player');
      expect(currentFunds).toBe(8.00); // $10.00 - $1.00 - $1.00 = $8.00
    });
    ...
    it('should prevent creation when insufficient funds for virtual dollar (Task 3.3)', () => {
      ...
      runOrchestrator.initializePlayer('poor-player', 0.50, 'BALANCED');
      ...
      const remainingFunds = (runOrchestrator as any).playerFunds.get('poor-player');
      expect(remainingFunds).toBe(0.50);
    });
  });
