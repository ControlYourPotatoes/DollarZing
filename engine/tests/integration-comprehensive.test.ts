...
// Uses SimulationController for proper orchestration and production-like testing
...
describe('Integration Test: Independent Run System with SimulationController', () => {
  ...
  beforeEach(() => {
    ...
    config = {
      ...
      initialDonationAmount: 20.00,
      maxSimulationTimeMs: 10000, // 10 second limit for tests
      enableProgressReporting: false // Disable for cleaner test output
    };

    // Initialize SimulationController with proper configuration
    simulationController = new SimulationController(config);
  ...
  describe('Task 8.1: SimulationController Integration', () => {
    test('should properly initialize all components through controller', () => {
      console.log('\n--- Testing SimulationController Component Integration ---');
      
      // The SimulationController should have initialized all components
      ...
      console.log('✓ SimulationController properly initialized all components');
    });
  });
  ...
  describe('Task 8.2: Multiple Independent Runs via Controller', () => {
    test('should handle simulation with multiple runs per player', async () => {
      ...
      console.log(`  Platform Revenue: $${result.revenueStats.totalPlatformRevenue.toFixed(2)}`);
      ...
      console.log(`  Charity Contributions: $${result.revenueStats.totalCharityContributions.toFixed(2)}`);
      ...
      console.log('✓ Multiple independent runs handled successfully by controller');
    });
  });
  ...
  describe('Task 8.3: Player Lifecycle Management', () => {
    test('should manage complete player lifecycle through controller', async () => {
      ...
      console.log(`  Final active players: ${result.playerStats.activePlayers}`);
  ...
  describe('Task 8.6: Jackpot Scenario Testing', () => {
    test('should validate jackpot calculations through progression', () => {
      ...
      console.log(`  Total possible winnings (all levels): $${totalPossibleWinnings}`);
  ...
  describe('Task 8.7: Investment Scenarios via Configuration', () => {
    test('should validate different player investment configurations', () => {
      ...
      const scenarios = [
        { amount: 5.00, expectedCredits: 4, description: '$5 investment' },
        { amount: 10.00, expectedCredits: 9, description: '$10 investment' },
        { amount: 20.00, expectedCredits: 18, description: '$20 investment' }
      ];
      ...
      for (const scenario of scenarios) {
        ...
        const actualCredits = Math.floor(scenario.amount / 1.10);
  ...
  describe('Task 8.11: Performance Testing with Controller', () => {
    test('should handle performance requirements through simulation controller', async () => {
      ...
      const perfConfig: SimulationConfig = {
        ...
        initialDonationAmount: 5.00, // Lower amount for more games
        maxSimulationTimeMs: 5000, // 5 second limit
        enableProgressReporting: false
      };

      const perfController = new SimulationController(perfConfig);
      
      console.log(`Performance test configuration:`);
      console.log(`  Players: ${perfConfig.initialPlayerCount}`);
      console.log(`  Donation per player: $${perfConfig.initialDonationAmount}`);
      ...
      console.log(`  Games per second: ${((result.gameStats.totalGames / executionTime) * 1000).toFixed(2)}`);
      console.log(`  Platform revenue: $${result.revenueStats.totalPlatformRevenue.toFixed(2)}`);
      ...
      expect(result.success).toBe(true);
      expect(result.gameStats.totalGames).toBeGreaterThanOrEqual(0);
      expect(result.playerStats.totalPlayers).toBe(perfConfig.initialPlayerCount);
  ...
  describe('Task 8.13: Audit Trail through Controller', () => {
    test('should maintain complete audit trail via simulation controller', async () => {
      ...
      console.log(`  Players created: ${result.playerStats.totalPlayers}`);
      console.log(`  Games completed: ${result.gameStats.totalGames}`);
      console.log(`  Revenue tracked: $${result.revenueStats.totalPlatformRevenue.toFixed(2)}`);
      console.log(`  Charity contributions: $${result.revenueStats.totalCharityContributions.toFixed(2)}`);
      ...
      expect(result.playerStats.totalPlayers).toBe(config.initialPlayerCount);
      expect(result.gameStats.totalGames).toBeGreaterThanOrEqual(0);
      expect(result.revenueStats.totalPlatformRevenue).toBeGreaterThanOrEqual(0);
      expect(result.completedAt).toBeInstanceOf(Date);

      // Revenue tracking should have comprehensive data
      expect(result.playerStats.activePlayers).toBeGreaterThanOrEqual(0);
      expect(result.revenueStats.totalCharityContributions).toBeGreaterThanOrEqual(0);
      expect(result.revenueStats.totalPlayerPayouts).toBeGreaterThanOrEqual(0);

      console.log('✓ Complete audit trail maintained through simulation controller');
    });
  });
  ...
  describe('Task 8.14: Configuration Validation', () => {
    test('should validate configuration parameters properly', () => {
      ...
      for (const { name, config: testConfig, expectedError } of invalidConfigs) {
        console.log(`Testing ${name}:`);
        
        expect(() => {
          new SimulationController(testConfig);
  ...
  describe('Task 8.15: Integration Test Summary', () => {
    test('should verify all integration tests pass with SimulationController', async () => {
      ...
      console.log('Verified systems (via SimulationController):');
      ...
      console.log(`Revenue: $${result.revenueStats.totalPlatformRevenue.toFixed(2)}`);
      console.log(`Execution: ${result.simulationDurationMs}ms`);

      console.log('\n✓ All integration tests pass with SimulationController architecture');
      console.log('✓ Virtual Dollar Pool Engine ready for production use');
      console.log('✓ Full system integration validated through proper orchestration');
    });
  });
});