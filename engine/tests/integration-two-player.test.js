...
import { CashOutStrategy, CashOutDecision, DollarState } from '../src/types/virtual-dollar-engine';
// Mock time for consistent testing
const mockTime = new Date('2025-08-24T10:00:00Z');
describe('Integration Test: 2 Player Independent Run Flow', () => {
    let playerBalanceManager;
    let virtualDollarManager;
    let scoringEngine;
    let gameMatchingEngine;
    let progressionManager;
    let revenueCalculator;
    const PLAYER_1_ID = 'player_001';
    const PLAYER_2_ID = 'player_002';
    const INITIAL_DONATION = 20.00; // $20 starting balance
    const DAILY_SEED = 'integration-test-2025-08-24';
    beforeEach(() => {
    ...
    describe('Task 8.1: Complete Independent Run Flow', () => {
        ...
        test('should process first level game between 2 players', async () => {
            ...
            expect(player1After.donationBalance).toBe(18.90); // $20 - $1.10
            expect(player2After.donationBalance).toBe(18.90); // $20 - $1.10
            // Execute the game
            ...
            if (gameResult) {
                ...
                console.log(`  Winner Score: ${gameResult.dollar1Score.toFixed(6)}`);
                console.log(`  Loser Score: ${gameResult.dollar2Score.toFixed(6)}`);
                console.log(`  Winnings: $${gameResult.winnings}`);
                // Verify game details
                expect(gameResult.level).toBe(1); // First level
                expect(gameResult.platformFee).toBe(0.20); // 20 cents platform fee
                expect(gameResult.winnings).toBe(2); // Level 1 winnings = $2
                expect([PLAYER_1_ID, PLAYER_2_ID]).toContain(gameResult.winner.ownerId);
        ...
        test('should track progression through multiple levels or cash-out decision', async () => {
            ...
            while (gameCount < maxGames) {
                ...
                gameCount++;
                console.log(`\n--- Game ${gameCount} (Level ${gameResult.level}) ---`);
                ...
                const lostAmount = playerBalanceManager.loseProgression(gameResult.loser.ownerId);
                ...
                const shouldCashOut = progressionManager.makeCashOutDecision(gameResult.winner, winnerPlayer.cashOutStrategy);
                ...
                if (shouldCashOut === CashOutDecision.CASH_OUT) {
                    ...
                    currentLevel++;
                    if (currentLevel <= 10) {
                        gameResult.winner.currentLevel = currentLevel;
                        virtualDollarManager.updateDollarState(gameResult.winner.id, DollarState.POOLED);
        ...
        test('should generate complete output summary for 2-player integration', async () => {
            ...
            console.log(`Created ${player1Dollars.length + player2Dollars.length} virtual dollars:`);
            ...
            while (Date.now() - startProcessing < maxProcessingTime) {
                ...
                if (!gameResult)
                    break;
                totalGames++;
                totalRevenue += gameResult.platformFee;
                ...
                const shouldCashOut = progressionManager.makeCashOutDecision(gameResult.winner, winnerPlayer.cashOutStrategy);
                if (shouldCashOut === CashOutDecision.CASH_OUT) {
                    const cashOut = playerBalanceManager.processCashOut(gameResult.winner.ownerId, 0.15);
            ...
            console.log(`Charity Contributions: $${totalCharityContributions.toFixed(2)}`);
            // Player final states
            ...
            console.log(`Completed (won/lost/cashed-out): ${poolStats.completedDollars}`);
...
//# sourceMappingURL=integration-two-player.test.js.map