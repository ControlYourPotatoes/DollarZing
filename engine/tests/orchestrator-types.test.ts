...
describe('Parameter Matrix Types', () => {
  ...
  it('should validate parameter combinations correctly', () => {
    // Valid combinations
    expect(validateParameterCombination({
      growthRate: 15,
      riskLevel: 'low',
      charityPercentage: 10
    })).toBe(true);
  ...
  it('should generate unique combinations without duplicates', () => {
    const combinations = generateAllCombinations();
    const uniqueCombinations = new Set(combinations.map(combo => 
  ...
  it('should cover all possible combinations systematically', () => {
    ...
    const charityPercentages = [...new Set(combinations.map(c => c.charityPercentage))];
...
describe('Orchestrator Configuration', () => {
  ...
  it('should validate configuration parameters', () => {
    expect(() => createDefaultOrchestratorConfig({
      batchSize: -1 // Invalid
