...
describe('GameMatchingEngine Factory Integration', () => {
  ...
  describe('Constructor Factory Injection', () => {
    ...
    it('should work with both Direct and Pooled factories', () => {
      ...
      const directFactory = new DirectGameSessionFactory(PRODUCTION_PERFORMANCE_CONFIG);
      const engineWithDirect = new GameMatchingEngine(dollarManager, scoringEngine, directFactory);
      expect(engineWithDirect).toBeDefined();

      // Test with Pooled factory (note: pooling may be disabled in test environment)
      const pooledFactory = new PooledGameSessionFactory(PRODUCTION_PERFORMANCE_CONFIG);
      const engineWithPooled = new GameMatchingEngine(dollarManager, scoringEngine, pooledFactory);
  ...
  describe('Game Session Lifecycle Management', () => {
    ...
    it('should release game sessions through factory after resolution', () => {
      ...
      // Resolve the game
      const resolutionResult = gameMatchingEngine.resolveGame(gameSession.id, '2024-01-15');
