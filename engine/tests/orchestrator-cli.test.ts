...
describe('CLI Argument Parsing', () => {
  ...
  it('should apply defaults for unspecified arguments', () => {
    ...
    expect(config.enableProgressReporting).toBe(defaults.enableProgressReporting);
  });
  ...
  it('should validate invalid numeric arguments', () => {
    const mockArgv = [
      'node',
      'orchestrator.js',
      '--batch-size', '-1'  // Invalid
    ];

    expect(() => parseCliArguments(mockArgv)).toThrow('Batch size must be positive');
  });
  ...
  it('should validate timeout values', () => {
    ...
    expect(() => parseCliArguments(mockArgv)).toThrow('Timeout must be positive');
  });
});
...
describe('CLI Configuration Validation', () => {
  ...
  it('should reject invalid output directory', () => {
    ...
    expect(() => validateCliConfiguration(invalidConfig)).toThrow('Output directory cannot be empty');
  });
  ...
  it('should reject invalid batch size', () => {
    ...
    expect(() => validateCliConfiguration(invalidConfig)).toThrow('Batch size must be positive');
  });
...
describe('CLI Program Creation', () => {
  ...
  it('should provide appropriate help text for each option', () => {
    ...
    const batchOption = options.find(opt => opt.flags.includes('--batch-size'));
...
describe('Parameter Matrix CLI Integration', () => {
  ...
  it('should support filtering combinations via CLI', () => {
    ...
    const lowGrowthCombinations = allCombinations.filter(combo => combo.growthRate === 15);
    expect(lowGrowthCombinations).toHaveLength(9); // 3 risk levels × 3 charity percentages
    
    // Test filtering by risk level
    const highRiskCombinations = allCombinations.filter(combo => combo.riskLevel === 'high');
