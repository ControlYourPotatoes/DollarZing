...
import {
  GameMatchingEngine,
  GameEvent,
} from "../src/types/game-matching-engine";
import {
...
describe("GameMatchingEngine", () => {
  ...
  describe("Virtual Dollar Pool Management", () => {
    ...
    it("should handle pool size limits gracefully", () => {
      ...
      expect(gameMatchingEngine.getPoolStatistics().totalDollarsInPool).toBe(
        1000
      );
    });
    ...
    it("should organize pool by betting levels efficiently", () => {
      ...
      levels.forEach((level) => {
        for (let i = 0; i < 5; i++) {
          const dollar = virtualDollarManager.createVirtualDollar(
            `player${level}_${i}`
          );
  ...
  describe("1v1 Matching Algorithm", () => {
    ...
    it("should handle odd number of dollars gracefully", () => {
      ...
      expect(gameMatchingEngine.getPoolStatistics().availableForMatching).toBe(
        1
      ); // 1 left over
    });
  ...
  describe("Game Resolution using Algorithmic Scores", () => {
    it("should resolve game using scoring engine", () => {
      ...
      expect([dollar1.serialNumber, dollar2.serialNumber]).toContain(
        gameResult.winner!.serialNumber
      );
      expect([dollar1.serialNumber, dollar2.serialNumber]).toContain(
        gameResult.loser!.serialNumber
      );
      expect(gameResult.winner!.serialNumber).not.toBe(
        gameResult.loser!.serialNumber
      );
    });
    ...
    it("should update dollar states after game resolution", () => {
      ...
      const winnerDollar = virtualDollarManager.getDollar(
    ...
    it("should calculate correct winnings for betting level", () => {
      ...
      testLevels.forEach((level) => {
        const dollar1 = virtualDollarManager.createVirtualDollar(
          `player1_${level}`
        );
        const dollar2 = virtualDollarManager.createVirtualDollar(
          `player2_${level}`
        );
        ...
        // Verify winnings match betting level value (2^(level-1) × 1.8 per game rules)
        const expectedWinnings = Math.pow(2, level - 1) * 1.8; // Level 1: $1.80, Level 2: $3.60, Level 3: $7.20, Level 4: $14.40, Level 5: $28.80...
  ...
  describe("Game Session Creation and Data Integrity", () => {
    it("should create complete game session with all required data", () => {
      ...
      expect(game.platformFee).toBe(0.2); // 20c
  ...
  describe("Game Event System", () => {
    ...
    it("should provide detailed event data for analytics", () => {
      ...
      expect(gameResolvedEvent!.winnings).toBe(7.2); // Level 3 = $7.20 (2^(3-1) × 1.8 = 4 × 1.8)
    });
  });
  ...
  describe("Comprehensive Game Tracking and Audit Trail", () => {
    ...
    it("should provide detailed statistics for monitoring", () => {
      ...
      expect(statistics.totalPlatformFees).toBeCloseTo(0.6); // 3 games × 20c
    });
  ...
  describe("PlayerBalanceManager Integration", () => {
    ...
    beforeEach(() => {
      ...
      mockPlayerBalanceManager = {
        ...
        canPlayerPlay(playerId: string): boolean {
          ...
          return player
            ? player.donationBalance >= 1.1 && player.isActive
            : false;
        },

        getPlayerGameCredits(playerId: string): number {
          const player = players.get(playerId);
          return player ? Math.floor(player.donationBalance / 1.1) : 0;
        },

        processGameFee(playerId: string): boolean {
          const player = players.get(playerId);
          if (player && player.donationBalance >= 1.1) {
            player.donationBalance -= 1.1;
            player.gamesPlayed++;
            return true;
          }
          return false;
        ...
        ): { playerAmount: number; charityAmount: number } {
          const player = players.get(playerId);
    ...
    it("should check player eligibility before allowing games", () => {
      ...
      mockPlayerBalanceManager.createPlayer(
        "player1",
        20.0,
    ...
    it("should process game fees when games are resolved", () => {
      const player = mockPlayerBalanceManager.createPlayer(
        "player1",
        20.0,
        CashOutStrategy.BALANCED
      );
      ...
      expect(player.donationBalance).toBe(initialBalance - 1.1);
    ...
    it("should handle player progression and winnings", () => {
      ...
      mockPlayerBalanceManager.addWinProgression("player1", 8.0); // Level 3 win

      expect(player.currentProgression).toBe(8.0);
    ...
    it("should process cash-outs with charity contributions", () => {
      ...
      mockPlayerBalanceManager.addWinProgression("player1", 16.0); // Level 4 win

      // Cash out with 15% charity
      const cashOutResult = mockPlayerBalanceManager.processCashOut(
        "player1",
        0.15
      );

      expect(cashOutResult.charityAmount).toBeCloseTo(2.4); // 16 * 0.15
      expect(cashOutResult.playerAmount).toBeCloseTo(13.6); // 16 * 0.85
      expect(player.winningsBalance).toBeCloseTo(13.6);
    ...
    it("should handle player losses correctly", () => {
      ...
      // Add progression winnings
      mockPlayerBalanceManager.addWinProgression("player1", 4.0);
      expect(player.currentProgression).toBe(4.0);

      // Lose progression (game loss)
      const lostAmount = mockPlayerBalanceManager.loseProgression("player1");

      expect(lostAmount).toBe(4.0);
    ...
    it("should track multiple players independently", () => {
      const player1 = mockPlayerBalanceManager.createPlayer(
        "player1",
        20.0,
        CashOutStrategy.CONSERVATIVE
      );
      const player2 = mockPlayerBalanceManager.createPlayer(
        "player2",
        30.0,
      ...
      mockPlayerBalanceManager.processGameFee("player1");
      mockPlayerBalanceManager.addWinProgression("player1", 2.0);

      mockPlayerBalanceManager.processGameFee("player2");
      mockPlayerBalanceManager.processGameFee("player2");
      mockPlayerBalanceManager.addWinProgression("player2", 4.0);

      expect(player1.donationBalance).toBeCloseTo(18.9);
      expect(player1.currentProgression).toBe(2.0);
      expect(player1.gamesPlayed).toBe(1);

      expect(player2.donationBalance).toBeCloseTo(27.8);
      expect(player2.currentProgression).toBe(4.0);
