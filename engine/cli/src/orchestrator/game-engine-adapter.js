...
import { validateParameterCombination, generateDirectoryName, generateFilePaths } from './parameter-matrix';
/**
...
export class GameEngineAdapter {
    config;
    orchestratorConfig;
    constructor(orchestratorConfig, mappingConfig) {
     ...
     */
    async generateDataset(combination, progressCallback) {
        ...
        try {
            ...
            if (!validateParameterCombination(combination)) {
                return {
                    combination,
                    success: false,
                    error: `Invalid parameter combination: ${JSON.stringify(combination)}`,
            ...
            if (this.orchestratorConfig.verbose) {
                console.log(`Initialized ${playersCreated} players for combination ${generateDirectoryName(combination)}`);
            }
            // Create progress wrapper if callback provided
            const wrappedCallback = progressCallback ?
                (progress) => progressCallback(combination, progress) :
                undefined;
            // Run simulation
            ...
            if (!simulationResults.success) {
                return {
                    combination,
                    success: false,
                    error: simulationResults.error || 'Simulation failed without specific error',
                    simulationResults,
                    generationTimeMs: performance.now() - startTime
                };
            }
            // Generate output paths
            const outputPaths = generateFilePaths(this.orchestratorConfig.outputDirectory, combination);
     ...
     */
    createSimulationConfig(combination) {
        const { growthRate, riskLevel, charityPercentage } = combination;
        // Apply growth rate scaling
        const playerCountMultiplier = this.config.growthRateScaling.playerCountMultiplier[growthRate];
        const durationMultiplier = this.config.growthRateScaling.durationMultiplier[growthRate];
        const adjustedPlayerCount = Math.round(this.config.basePlayerCount * playerCountMultiplier);
        const adjustedDuration = Math.round(this.config.baseSimulationDays * durationMultiplier);
    ...
    createMappingConfig(overrides) {
        const defaults = {
            ...
            riskStrategyMapping: {
                low: {
                    'low': 0.7, // 70% conservative cash-out
                    'average': 0.25, // 25% balanced cash-out
                    'high': 0.05 // 5% aggressive cash-out
                },
                mid: {
                    'low': 0.3, // 30% conservative cash-out
                    'average': 0.5, // 50% balanced cash-out
                    'high': 0.2 // 20% aggressive cash-out
                },
                high: {
                    'low': 0.1, // 10% conservative cash-out
                    'average': 0.3, // 30% balanced cash-out
                    'high': 0.6 // 60% aggressive cash-out
                }
            }
        };
     ...
     */
    createDatasetMetadata(combination, simulationResults, generationTimeMs, datasetSizeBytes) {
     ...
     */
    calculateRecordCount(simulationResults) {
        // Estimate based on days simulated and average daily activity
        const { gameStats, config } = simulationResults;
        return Math.round(config.durationDays * gameStats.averageGamesPerDay +
    ...
    validateSimulationResults(simulationResults) {
        ...
        if (playerStats.activePlayers + playerStats.retiredPlayers !== playerStats.totalPlayers) {
     ...
     */
    updateMappingConfig(updates) {
        this.config = { ...this.config, ...updates };
    }
    /**
     * Create a dataset generation result for testing/mocking
     */
    static createMockResult(combination, success = true) {
        if (!success) {
            return {
                combination,
                success: false,
        ...
        const mockResults = {
            ...
            config: {
                ...
                playerStrategies: { 'low': 0.5, 'average': 0.3, 'high': 0.2 },
        ...
        return {
            ...
            outputPaths: {
                ...
                datasetFile: `test/anchor-datasets/${generateDirectoryName(combination)}/dataset.json`,
                metadataFile: `test/anchor-datasets/${generateDirectoryName(combination)}/metadata.json`
            }
        };
    }
}
//# sourceMappingURL=game-engine-adapter.js.map