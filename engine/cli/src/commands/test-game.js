...
import { SimulationController } from '../../src/index.js';
/**
 * Create the test-game command
 */
export function createTestGameCommand() {
    ...
    command
        ...
        .option('--seed <string>', 'Random seed for reproducible results', 'test-game')
        .action(async (options) => {
        try {
            await executeTestGame(options);
        }
        catch (error) {
            console.error('❌ Test game failed:', error instanceof Error ? error.message : error);
            process.exit(1);
        }
    });
    // Add examples
 ...
 */
async function executeTestGame(options) {
    ...
    const charityPercentage = Math.max(0, Math.min(100, parseInt(options.charity || '20', 10)));
    ...
    const results = await controller.runSimulation(verbose ? (progress) => {
    ...
    if (results.success) {
        ...
        console.log(`  Per Day: ${results.gameStats.averageGamesPerDay.toFixed(1)}`);
        console.log(`  Virtual Dollars: ${results.gameStats.totalVirtualDollars}`);
        console.log(`  Jackpots Won: ${results.gameStats.jackpotsWon}`);
        console.log('');
        console.log('💰 Revenue:');
        console.log(`  Platform: $${results.revenueStats.totalPlatformRevenue.toFixed(2)}`);
        console.log(`  Charity: $${results.revenueStats.totalCharityContributions.toFixed(2)} (${(results.revenueStats.charityPercentage * 100).toFixed(1)}%)`);
        console.log(`  Player Payouts: $${results.revenueStats.totalPlayerPayouts.toFixed(2)}`);
        console.log(`  Per Game: $${results.revenueStats.revenuePerGame.toFixed(2)}`);
        if (verbose) {
            console.log('');
            console.log('📊 Detailed Statistics:');
            console.log(`  Average Games per Player: ${results.playerStats.averageGamesPerPlayer.toFixed(1)}`);
            console.log(`  Player Retirement Rate: ${(results.playerStats.playerRetirementRate * 100).toFixed(1)}%`);
            console.log(`  Average Run Length: ${results.gameStats.averageRunLength.toFixed(1)} games`);
            ...
            console.log(`  Donation Funds: $${results.playerStats.totalDonationsFunds.toFixed(2)}`);
            console.log(`  Winnings Funds: $${results.playerStats.totalWinningsFunds.toFixed(2)}`);
            console.log(`  Progression Funds: $${results.playerStats.totalProgressionFunds.toFixed(2)}`);
        }
        console.log('');
 ...
 */
export function validateTestGameOptions(options) {
    const errors = [];
