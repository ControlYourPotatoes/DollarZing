// Player Run Manager - Manages individual player runs and virtual dollar lifecycle
// Handles the flow from run creation through completion and new run initiation
...
// ===== PLAYER RUN MANAGER CLASS =====

/**
 * PlayerRunManager - Manages individual player runs and virtual dollar lifecycle
 * Handles the complete lifecycle from run creation through completion and new run initiation
 */
export class PlayerRunManager {
  ...
  constructor(
    ...
    this.progressionManager =
      progressionManager || new ProgressionManager(charityPercentage);
  ...
  createNewRun(request: NewRunRequest): VirtualDollar | null {
    ...
    const virtualDollarCost = 1.0; // Only cost of creating virtual dollar - no game fees
    ...
    const virtualDollar = this.dollarManager.createVirtualDollar(
      request.playerId
    );
  ...
  processGameResult(
    ...
    console.log(
      `DEBUG: Processing ${result} for player ${playerId} at level ${virtualDollar.currentLevel}`
    );
    ...
    if (result === GameResult.WIN) {
      ...
      const nextLevelCost = nextLevel + 0.2; // Next bet + platform fee

      if (currentFunds + winnings < nextLevelCost && nextLevel <= 512) {
        // Player can't afford next level - force cash out
        const completionResult = this.progressionManager.processCashOut(
          virtualDollar.runId
        );
        return this.handleRunCompletion(
          virtualDollar,
          "CASH_OUT",
          completionResult
        );
    ...
    const progression = this.progressionManager.processGameResult(
      virtualDollar,
      result
    );
    ...
    if (progression.isComplete) {
      return this.handleRunCompletion(
        virtualDollar,
        progression.completionReason!
      );
    }

    // Run continues - check if player wants to cash out
    const strategy =
      this.playerStrategies.get(virtualDollar.ownerId) ||
      CashOutStrategy.BALANCED;
    const cashOutDecision = this.progressionManager.makeCashOutDecision(
      virtualDollar,
      strategy
    );
  ...
  private handleRunCompletion(
    ...
    const result =
      completionResult ||
      this.progressionManager.getCompletedRun(virtualDollar.runId)!;
    ...
    const newState =
      completionType === "LOSS" ? DollarState.LOST : DollarState.CASHED_OUT;
    this.dollarManager.updateDollarState(virtualDollar.id, newState);

    // Determine if player should create a new run
    const level1Cost = 1.2; // $1.00 bet + $0.20 platform fee for level 1
  ...
  getPlayerRunContext(playerId: string): PlayerRunContext {
    const activeDollars = this.dollarManager
      .getDollarsByPlayer(playerId)
      .filter(
        (dollar) =>
    ...
    const completedRunIds = this.dollarManager
      .getDollarsByPlayer(playerId)
      .filter(
        (dollar) =>
          dollar.state === DollarState.LOST ||
  ...
  getAllActiveRuns(): VirtualDollar[] {
    return this.dollarManager.getPooledDollars().concat(
      this.progressionManager
        .getActiveRuns()
        .map((run) => this.dollarManager.getDollarByRunId(run.runId))
  ...
  getActivePlayerCount(): number {
    const playersWithFunds = Array.from(this.playerFunds.entries()).filter(
      ([_, funds]) => funds > 0
    ).length;
