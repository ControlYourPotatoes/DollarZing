...
export function validateVirtualDollar(dollar: VirtualDollar): ValidationResult {
  ...
  if (dollar.potValue < 0.9) {
    warnings.push(
      "Pot value below $0.90 may indicate missing platform fee logic"
    );
  }
...
export function validateSimulationParameters(
  ...
  const total =
    params.cashOutStrategyDistribution.conservative +
  ...
  if (params.charityPercentage < 0.1 || params.charityPercentage > 1.0) {
...
/**
 * Calculate winning amount for a betting level (bet amount × 1.8)
 */
...
export interface GameTransaction {
  ...
  balanceAfter: {
    // Player balance after transaction
