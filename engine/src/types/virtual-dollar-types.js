...
export class VirtualDollarManager {
    ...
    pooledDollars = new Set();
    dollarsByPlayer = new Map();
    ...
    createVirtualDollar(playerId) {
        ...
        if (isObjectPoolingEnabled()) {
            ...
            poolManager.virtualDollarPool.initializeDollar(dollar, this.generateUniqueId(), serialNumber, playerId, this.generateRunId());
        }
        else {
     ...
     */
    updateDollarState(dollarId, newState) {
     ...
     */
    updatePoolTracking(dollarId, state) {
     ...
     */
    getDollarBySerial(serialNumber) {
     ...
     */
    createBatchVirtualDollars(playerIds) {
        const dollars = [];
     ...
     */
    getDollarStateHistory(dollarId) {
     ...
     */
    updateRunData(dollarId, winnings) {
        const dollar = this.dollars.get(dollarId);
        if (!dollar) {
            return {
                isValid: false,
     ...
     */
    getActiveRunsByPlayer(playerId) {
        const playerDollars = this.getDollarsByPlayer(playerId);
        return playerDollars.filter(dollar => dollar.state !== DollarState.CASHED_OUT &&
            dollar.state !== DollarState.LOST);
    }
    /**
     * Get completed runs for a player
     */
    getCompletedRunsByPlayer(playerId) {
        const playerDollars = this.getDollarsByPlayer(playerId);
        return playerDollars.filter(dollar => dollar.state === DollarState.CASHED_OUT ||
            dollar.state === DollarState.LOST);
    }
     ...
     */
    cleanupCompletedDollars() {
        const completedDollars = [];
...
export { DollarState };
//# sourceMappingURL=virtual-dollar-types.js.map