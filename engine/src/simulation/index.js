...
export class PlayerBehaviorModel {
    ...
    getCashOutProbability(level, strategy) {
        switch (strategy) {
            case "low":
...
export class GrowthCalculator {
    calculateOrganicGrowth(day, currentPlayers, growthMultiplier, randomSeed) {
    ...
    calculateAdoptionGrowth(day, currentPlayers, adoptionRate, randomSeed) {
        const rng = new SeededRandom(day * 1000 + Math.floor(randomSeed * 10000));
...
export class LevelProgressionCalculator {
    ...
    calculateLevelStatistics(level, gamesAtLevel, strategy, randomSeed) {
        const rng = new SeededRandom(level * 1000 + Math.floor(randomSeed * 10000));
        const successRate = level === 10
            ? 1.0
...
export class RevenueDistributionCalculator {
    calculateRevenueBreakdown(amount, flowType) {
...
export class SimulationEngine {
    ...
    async simulateDay(day, totalPlayers, activePlayers) {
        ...
        if (this.parameters.adoptionRate > 0) {
            newActivePlayers = this.growthCalculator.calculateAdoptionGrowth(day, activePlayers, this.parameters.adoptionRate, rng.next());
        }
        else {
            newActivePlayers = this.growthCalculator.calculateOrganicGrowth(day, activePlayers, this.parameters.growthMultiplier, rng.next());
        }
        ...
        for (let levelIndex = 0; levelIndex < LEVELS.length; levelIndex++) {
            ...
            // Distribute revenue
            const breakdown = this.revenueCalculator.calculateRevenueBreakdown(winningsToDistribute, level === 10 ? "jackpot" : "cashout");
            ...
            const levelStats = this.progressionCalculator.calculateLevelStatistics(level, gamesAtCurrentLevel, this.parameters.cashOutStrategy, rng.next());
    ...
    generateYearlyAggregation(dailySnapshots) {
        ...
        // Find peak day
        const peakDay = dailySnapshots.reduce((peak, snapshot) => snapshot.financialMetrics.totalRevenue >
