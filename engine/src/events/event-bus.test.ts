...
describe('EventBus', () => {
  ...
  describe('Basic Event Handling', () => {
    it('should subscribe and emit events correctly', async () => {
      ...
      const testEvent = { type: 'TEST_EVENT', data: 'test data', timestamp: new Date() };
    ...
    it('should handle multiple subscribers for the same event', async () => {
      ...
      const testEvent = { type: 'TEST_EVENT', data: 'test', timestamp: new Date() };
    ...
    it('should not call handlers for different event types', async () => {
      ...
      await eventBus.emit('EVENT_A', { type: 'EVENT_A', timestamp: new Date() });
  ...
  describe('Priority Ordering', () => {
    it('should execute handlers in priority order (higher numbers first)', async () => {
      ...
      await eventBus.emit('TEST_EVENT', { type: 'TEST_EVENT', timestamp: new Date() });
  ...
  describe('Async Handler Support', () => {
    ...
    it('should wait for all async handlers to complete', async () => {
      let completed = 0;
      const createAsyncHandler = (delay: number) => vi.fn(async () => {
  ...
  describe('Event Tracing', () => {
    it('should trace events when tracing is enabled', async () => {
      const testEvent = { type: 'TEST_EVENT', data: 'trace test', timestamp: new Date() };
    ...
    it('should not trace events when tracing is disabled', async () => {
      ...
      await noTraceEventBus.emit('TEST_EVENT', { type: 'TEST_EVENT', timestamp: new Date() });
  ...
  describe('Listener Limits', () => {
    it('should enforce maximum listener limit', () => {
      ...
      expect(() => limitedEventBus.on('TEST_EVENT', vi.fn()))
        .toThrow('Maximum listener limit (2) reached for event type: TEST_EVENT');
  ...
  describe('Memory Management', () => {
    it('should dispose all listeners and traces', () => {
      const handler = vi.fn();
      eventBus.on('TEST_EVENT', handler);
      eventBus.emit('TEST_EVENT', { type: 'TEST_EVENT', timestamp: new Date() });
