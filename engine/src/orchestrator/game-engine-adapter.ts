...
export class GameEngineAdapter {
  ...
  async generateDataset(
    ...
    try {
      ...
      const wrappedCallback = progressCallback ? 
        (progress: SimulationProgress) => progressCallback(combination, progress) : 
        undefined;
   ...
   */
  private createSimulationConfig(combination: ParameterCombination): SimulationConfig {
   ...
   */
  private createMappingConfig(overrides?: Partial<ParameterMappingConfig>): ParameterMappingConfig {
   ...
   */
  validateSimulationResults(simulationResults: SimulationResults): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
   ...
   */
  static createMockResult(combination: ParameterCombination, success: boolean = true): AdapterGenerationResult {
    if (!success) {
      return {
        combination,
        success: false,
