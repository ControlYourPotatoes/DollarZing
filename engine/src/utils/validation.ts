...
export function validateSimulationParameters(
  ...
  if (params.adoptionRate < 0 || params.adoptionRate > 1) {
    errors.push("Invalid adoption rate");
  ...
  if (params.growthMultiplier < 0) {
    errors.push("Invalid growth multiplier");
  } else if (params.growthMultiplier > 5) {
    warnings.push(
      "Growth multiplier above 5x may lead to unrealistic projections"
    );
  }
  ...
  if (!validStrategies.includes(params.cashOutStrategy)) {
    errors.push("Invalid cash-out strategy");
  }
