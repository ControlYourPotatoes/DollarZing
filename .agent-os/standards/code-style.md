...
### Go
- **Variables and Functions**: Use camelCase for unexported (e.g., `userProfile`, `calculateTotal`)
- **Exported Functions**: Use PascalCase (e.g., `UserProfile`, `CalculateTotal`)
- **Constants**: Use camelCase or PascalCase based on visibility (e.g., `maxRetryCount`, `APIBaseURL`)
- **Package Names**: Use lowercase, single word when possible (e.g., `user`, `payment`)
...
- **Constants**: Use UPPER_SNAKE_CASE (e.g., `MAX_RETRY_COUNT`, `API_BASE_URL`)
- **Files**: Use snake_case (e.g., `user_profile.py`, `payment_processor.py`)

### Database (PostgreSQL)
- **Tables**: Use snake_case, plural (e.g., `users`, `payment_transactions`)
- **Columns**: Use snake_case (e.g., `user_id`, `created_at`, `total_amount`)
- **Indexes**: Use descriptive snake_case (e.g., `idx_users_email`, `idx_payments_status`)
...
- We use a unique multi-line formatting style when writing Tailwind CSS classes in HTML markup and JSX components, where the classes for each responsive size are written on their own dedicated line.
...
// Uses the formula: A = P(1 + r/n)^(nt) + PMT × (((1 + r/n)^(nt) - 1) / (r/n))
func CalculateCompoundInterest(principal, rate, time, monthlyPayment float64) float64 {
    // Implementation here
}
```
