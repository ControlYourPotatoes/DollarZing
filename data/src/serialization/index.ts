...
function createJsonReplacer(options: SerializationOptions) {
...
function createJsonReviver() {
  return function(_key: string, value: any): any {
...
): { serialized: string; metrics: PerformanceMetrics } {
...
export function deserializeDataset(
  serialized: string
): { dataset: SimulationDataset; metrics: PerformanceMetrics } {
  const startTime = performance.now();
  
  // Deserialize with custom reviver
  const reviver = createJsonReviver();
  const dataset = JSON.parse(serialized, reviver) as SimulationDataset;
...
function compressDataset(dataset: SimulationDataset): SimulationDataset {
...
export function serializeDatasetBatch(
  datasets: SimulationDataset[], 
  options: Partial<SerializationOptions> = {}
): { serializedBatch: string; totalMetrics: PerformanceMetrics } {
  const startTime = performance.now();
...
export function serializeDatasetIncremental(
  dataset: SimulationDataset,
  chunkSize: number = 30, // Days per chunk
  options: Partial<SerializationOptions> = {}
): { chunks: string[]; totalMetrics: PerformanceMetrics } {
  const startTime = performance.now();
  const chunks: string[] = [];
  let totalMemoryUsage = 0;

  // Split daily snapshots into chunks
  const dailyChunks: DailySnapshot[][] = [];
  ...
  dailyChunks.forEach((chunk, index) => {
    const chunkDataset: Partial<SimulationDataset> = {
      ...dataset,
    ...
    const { serialized, metrics } = serializeDataset(chunkDataset as SimulationDataset, options);
...
export function estimateDatasetSize(dataset: SimulationDataset): {
  ...
  const totalPlayerJourneys = dataset.dailySnapshots.reduce(
    (sum, snapshot) => sum + snapshot.playerJourneys.length, 0
  );
