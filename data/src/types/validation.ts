...
// Dataset validation
export function validateSimulationDataset(dataset: SimulationDataset): DatasetValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
...
export function validateDailySnapshot(snapshot: DailySnapshot): DatasetValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
...
// Player metrics validation
export function validatePlayerMetrics(metrics: PlayerMetrics): DatasetValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
  ...
  for (const field of fields) {
    if (metrics[field as keyof PlayerMetrics] < 0) {
...
// Financial metrics validation
export function validateFinancialMetrics(metrics: FinancialMetrics): DatasetValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
  ...
  for (const field of fields) {
    if (metrics[field as keyof FinancialMetrics] < 0) {
      errors.push(`${field} cannot be negative`);
    }
  }
...
export function validateRevenueBreakdown(breakdown: RevenueBreakdown): DatasetValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];

  // Non-negative amounts
  const categories = ['platform', 'charity', 'government', 'players'] as const;
